...
{unc_doc_freq.df %>% 
  ...
  geom_col() +
  coord_flip() +
  labs(x="%age of dissertation assigned to controlled subjects (excluding stopwords, min 10%)")} %>% plotly::ggplotly()
...
hand_codes_tmp.df %>% select(-EpiStyle_type) %>%
    mutate(across(everything(), ~ case_when(is.na(.x) ~ TRUE,  .default = FALSE))) %>%
