...
stem_post.df <- stem_raw.df %>%
  dplyr::rowwise() %>%
  mutate( root = stringr::str_split_1(`groups`, pattern="; ")[[1]],
          stem = stringr::str_split(`groups`, pattern="; ")) %>%
  dplyr::ungroup() %>%
  tidyr::unnest(stem) %>%
...
combined_tf_idf_stemmed_only.df <- combined_tf_idf.df %>%
  slice_sample(n=1000) %>%
  rowwise() %>%
  mutate(ngram=stem_string_V(ngram)) %>%
  ungroup()
...
keyATM::semantic_coherence(x, docs, n = 10)
